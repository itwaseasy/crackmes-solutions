    ...
    def run(self, filename, offset, size):
        ...
        while index < len(self.code):
            try:
                ...
                bytecode_hex_str = f'{self.code[index:index + instruction.length()].hex(" "):50}'
                print(bytecode_hex_str)

                for b in str(instruction).split('\n'):
                    print(b.rjust(len(bytecode_hex_str) + len(b)))
